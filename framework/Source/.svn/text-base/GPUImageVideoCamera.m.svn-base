...
- (void)switchFlash {
    ...
    BOOL bCameraSound = [[NSUserDefaults standardUserDefaults] boolForKey:@"sound"];
    BOOL bFlashMode = [[NSUserDefaults standardUserDefaults] boolForKey:@"flashMode"];
    ...
    if (bCameraSound) {
        if (bFlashMode) {
            if ([_inputCamera hasFlash]) {
                _inputCamera.flashMode = AVCaptureFlashModeOn;
            }
        } else {
            if ([_inputCamera hasFlash]) {
                _inputCamera.flashMode = AVCaptureFlashModeOff;
            }
        }
    }
...
- (void)flashOn {
    ...
    if (!bCameraSound) {
        ...
        if ([_inputCamera hasTorch]) {
            _inputCamera.torchMode = AVCaptureTorchModeOn;
        }
        [_inputCamera unlockForConfiguration];
    }
}
...
- (void)flashOff {
    ...
    if (!bCameraSound) {
        [_inputCamera lockForConfiguration:nil];
        if ([_inputCamera hasTorch]) {
            _inputCamera.torchMode = AVCaptureTorchModeOff;
        }
        [_inputCamera unlockForConfiguration];
    }
}
