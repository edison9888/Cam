...
//  MMOpenCV
//
//  Created by Moon Sik on 11. 10. 14..
//  Copyright (c) 2011 inamass. All rights reserved.
//
// calibration_artificial.cpp : 콘솔응용프로그램에대한진입점을정의합니다.
//

#ifndef __CHESSBOARD_GENERATOR_HPP__
#define __CHESSBOARD_GENERATOR_HPP__
...
namespace cv
{
    ...
    public:
        ...
        ChessBoardGenerator(const cv::Size& patternSize = cv::Size(8, 6));
        cv::Mat operator()(const cv::Mat& bg, const cv::Mat& camMat, const cv::Mat& distCoeffs, vector<Point2f>& corners) const;   
        cv::Size cornersSize() const;
    private:
        void generateEdge(const Point3f& p1, const Point3f& p2, vector<Point3f>& out) const;
        cv::Mat generageChessBoard(const cv::Mat& bg, const cv::Mat& camMat, const cv::Mat& distCoeffs,
        ...
        Point3f generateChessBoardCenter(const cv::Mat& camMat, const cv::Size& imgSize) const;
        cv::Mat rvec, tvec;
    };
};
...
const cv::Size imgSize(800, 600);
const cv::Size brdSize(8, 7);
const size_t brds_num = 20;

template<class T> ostream& operator<<(ostream& out, const cv::Mat_<T>& mat)
{   
...
ChessBoardGenerator::ChessBoardGenerator(const cv::Size& _patternSize) : sensorWidth(32), sensorHeight(24),
squareEdgePointsNum(200), min_cos(sqrt(2.f)*0.5f), cov(0.5),
patternSize(_patternSize), rendererResolutionMultiplier(4), tvec(cv::Mat::zeros(1, 3, CV_32F))
{   
    Rodrigues(cv::Mat::eye(3, 3, CV_32F), rvec);
}
...
cv::Size cv::ChessBoardGenerator::cornersSize() const
{
...
cv::Mat cv::ChessBoardGenerator::generageChessBoard(const Mat& bg, const Mat& camMat, const Mat& distCoeffs,
                                                const Point3f& zero, const Point3f& pb1, const Point3f& pb2,
...
cv::Mat cv::ChessBoardGenerator::operator ()(const Mat& bg, const Mat& camMat, const Mat& distCoeffs, vector<Point2f>& corners) const
{     
    cov = min(cov, 0.8);
